...
NUM_KEYPOINTS = 9
KEYPOINTS_PER_ROW = NUM_KEYPOINTS * 3

# 对每个目录，只处理前5个文件
total_files = 0
for label_dir in label_dirs:
    # 获取所有标签文件
    label_files = glob.glob(os.path.join(label_dir, '*.txt'))[:5]  # 只处理前5个文件
    ...
    for label_file in label_files:
        ...
        print(f"  示例修复结果:")
        for i, fixed_line in enumerate(fixed_lines[:2]):
            print(f"    行{i+1}: {fixed_line[:100]}... (总长度: {len(fixed_line.split())})")

print(f"\n测试完成！共处理 {total_files} 个文件")